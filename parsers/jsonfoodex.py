'''Parsing suite for the Cookbase platform from FoodEx2 XML data into a JSON format.

The main command, `parsexml`, allows for lossless translation from FoodEx2 XML
data into JSON documents. Nonetheless, it also permits to filter out and
discard the desired hierarchies together with the ingredients that belong only
to those hierarchies. Field contents are parsed into Python built-in types
(`str`, `int` and `bool`). The original ordering and format are respected,
however there are a number of particularities when mapping into JSON to be
considered:
...
def parsexml(args: argparse.Namespace) -> None:
    ...
    catalogue["parserInfo"] = {
        "discardedHierarchies": args.discardedhierarchies}
    ...
    for i in node.childNodes:
        if i.nodeType != i.TEXT_NODE:
            if i.nodeName == "termCodeLength":
                catalogue["catalogueDesc"][i.nodeName] = int(
    ...
    catalogue["catalogueGroups"][node.childNodes[1]
                                 .nodeName] = node.childNodes[1].firstChild.nodeValue
    ...
    for hierarchy_node in hierarchy_nodes:
        code = hierarchy_node.getElementsByTagName(
    ...
    for attribute_node in attribute_nodes:
        code = attribute_node.getElementsByTagName(
            "code")[0].firstChild.nodeValue
        ...
        for i in attribute_node.childNodes:
            if i.nodeType != i.TEXT_NODE:
                ...
                for j in i.childNodes:
                    if j.nodeType != j.TEXT_NODE:
                        if j.nodeName in ["attributeOrder",
                                          "attributeMaxLength"]:
    ...
    for term_node in term_nodes:
        ...
        termCode = term_node.getElementsByTagName(
            "termCode")[0].firstChild.nodeValue
        ...
        for i in term_node.childNodes:
            if i.nodeType != i.TEXT_NODE:
                ...
                if i.nodeName == "hierarchyAssignments":
                    for j in i.childNodes:
                        if j.nodeType != j.TEXT_NODE:
                            hierarchy_code = j.getElementsByTagName(
                                "hierarchyCode")[0].firstChild.nodeValue
                            # updating counter
                            ...
                            for k in j.childNodes:
                                if k.nodeType != k.TEXT_NODE:
                                    if k.nodeName == "order":
                                        term_dict[i.nodeName][hierarchy_code][k.nodeName] = int(
    ...
    vprint(
        "Number of terms in the Exposure Hierarchy: " +
...
def _traverse_hierarchy(hierarchy_siblings: Dict[str, Any], term_to_attach: Optional[Tuple[str, str, Dict[str, Any]]] = None) -> bool:
    ...
    :return: `True` if a term was attached to hierarchy, `False` otherwise
    :rtype: bool
    '''
...
def hierarchize(args: argparse.Namespace) -> None:
    ...
    for e in os.scandir(args.inputfolder):
        if e.path.endswith(".json"):
            with open(e.path) as f:
                term = json.load(f)
            try:
                parent_code = term["hierarchyAssignments"][args.hierarchycode]["parentCode"]
            except KeyError:
                continue
            tta = (parent_code, term["termDesc"]["termExtendedName"], {"termCode": term["termDesc"]["termCode"], "reportable": term[
                   "hierarchyAssignments"][args.hierarchycode]["reportable"], "order": term["hierarchyAssignments"][args.hierarchycode]["order"]})
            if parent_code == "root":
    ...
    vprint("Time elapsed: " + str(ceil(time() - start_time)) + " seconds")
    vprint(
        "Total number of terms in hierarchy: " +
        str(hierarchy_term_counter))
...
def _main() -> None:
    ...
    parsexml_parser.add_argument(
        "inputfile", help="path to the XML input file")
    parsexml_parser.add_argument(
        "outputfile",
    ...
    pg.add_argument("-n", "--nchunks", type=int,
                    help="number of chunks to split the terms output file")
    pg.add_argument("-s", "--single", action="store_true",
                    help="indicate output terms in single files")
    parsexml_parser.add_argument("-v", "--verbose", action="count",
                                 default=0, help="increase output verbosity")
    parsexml_parser.add_argument("-d", "--discardedhierarchies", nargs='*',
                                 default=["botanic", "pest", "biomo", "legis", "feed", "partcon", "place",
                                          "vetdrug", "report", "fpurpose", "replev", "targcon", "feedAddExpo"],
    ...
    parsexml_parser.add_argument("-cb", "--cookbase", action="store_true",
    ...
    hierarchize_parser.add_argument(
        "inputfolder",
        help="path to the directory including single term files")
    hierarchize_parser.add_argument(
        "outputfile", help="path to the JSON output file")
    hierarchize_parser.add_argument(
        "hierarchycode",
        help="code of the hierarchy to build")
    hierarchize_parser.add_argument("-v", "--verbose", action="count",
                                    default=0, help="increase output verbosity")
    hierarchize_parser.set_defaults(func=hierarchize)
    args = ap.parse_args()
    # checking command-line arguments correctness
    if args.command == "parsexml":
        if (args.nchunks is not None or args.single ==
                True) and args.termsfile == None:
            ap.error(
                "-n/--nchunks and -s/--single can only be used if -t/--termsfile is declared")
