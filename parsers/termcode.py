'''A module allowing to generate and translate numeric identifiers from the
FoodEx2 `term code` strings. A `term code` consists of a string of five
alphanumeric characters, e. g. 'A111J'. While most of the times they start
with an 'A' character, this module does not restrict to that.
...
def _char_to_dec(character: str) -> int:
    ...
    if (ord(c) < 48 or ord(c) > 57) and (ord(c) < 65 or ord(c) > 90):
        raise ValueError("expected an alphanumeric character ([0-9], [A-Z], [a-z]), got '" + character + "' instead")
...
def _to_ascii(number: int) -> str:
    if number < 0 or number > 35:
        raise ValueError("expected an integer in the range (0, 35), got '" + str(number) + "' instead")
...
def to_str(code: int) -> str:
    '''Function generating a FoodEx2 `term code` from its numeric
    representation.
