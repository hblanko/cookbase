...
def parsexml(args: argparse.Namespace) -> None:
    ...
    if args.cookbase:
        try:
            ...
            print('   WARNING: cookbase.parsers.termcode is not included under' +
    ...
    catalogue['catalogueGroups'][node.childNodes[1].nodeName] = node.childNodes[1] \
                        ...
                        for j in i.childNodes:
                            if j.nodeType != j.TEXT_NODE:
                                if j.nodeName == 'hierarchyOrder':
                                    hierarchy_dict[i.nodeName][j.nodeName] = \
                                        int(j.firstChild.nodeValue)
                ...
                for j in i.childNodes:
                    if j.nodeType != j.TEXT_NODE:
                        if j.nodeName in ['attributeOrder',
                                          'attributeMaxLength']:
                            attribute_dict[i.nodeName][j.nodeName] = \
                                int(j.firstChild.nodeValue)
                        elif j.nodeName in ['attributeVisible',
                                            'attributeSearchable',
                                            'attributeUniqueness',
                        ...
                        if j.nodeType != j.TEXT_NODE:
                            ...
                            for k in j.childNodes:
                                if k.nodeType != k.TEXT_NODE:
                                    if k.nodeName == 'order':
                                        term_dict[
                                            i.nodeName][hierarchy_code][k.nodeName] = \
                                            int(k.firstChild.nodeValue)
                                    elif k.nodeName == 'reportable':
                                        if k.firstChild.nodeValue == 'true':
                                            term_dict[
                                                i.nodeName
                                            ][hierarchy_code][k.nodeName] = True
                                        else:
                                            term_dict[
                                                i.nodeName
                                            ][hierarchy_code][k.nodeName] = False
                                    else:
                                        term_dict[
                                            i.nodeName
                                        ][hierarchy_code][k.nodeName] = \
                        ...
                        if j.nodeType != j.TEXT_NODE:
                            ...
                            for k in j.childNodes:
                                if k.nodeName == 'attributeValues':
                                    ...
                                    term_dict[i.nodeName][attributeCode] = \
                                        attributeValues_list
                else:
                    for j in i.childNodes:
                        if j.nodeType != j.TEXT_NODE:
    ...
    if args.termsfile:
        if args.single:
            for i in catalogue_terms:
                with open(args.termsfile +
                          '/' + i['termDesc']['termCode'] + '.json', 'w') as f:
...
def _traverse_hierarchy(hierarchy_siblings: Dict[str, Any],
                        term_to_attach: Optional[Tuple[str, str, Dict[str, Any]]] =
                        None) -> bool:
...
def hierarchize(args: argparse.Namespace) -> None:
    ...
    for e in os.scandir(args.inputfolder):
        if e.path.endswith('.json'):
            ...
            try:
                parent_code = \
                    term['hierarchyAssignments'][args.hierarchycode]['parentCode']
            except KeyError:
                continue

            tta = (parent_code, term['termDesc']['termExtendedName'], {
                'termCode': term['termDesc']['termCode'], 'reportable': term[
                   'hierarchyAssignments'][args.hierarchycode]['reportable'],
...
def _main() -> None:
    ...
    parsexml_parser.add_argument('-t', '--termsfile',
    ...
    parsexml_parser.add_argument('-d', '--discardedhierarchies', nargs='*',
                                 default=['botanic', 'pest', 'biomo', 'legis', 'feed',
                                          'partcon', 'place', 'vetdrug', 'report',
                                          'fpurpose', 'replev', 'targcon',
                                          'feedAddExpo'],
    ...
    parsexml_parser.add_argument('-cb', '--cookbase', action='store_true',
                                 help='activate identifier generation (\'_id\') for ' +
                                 'Cookbase platform')
    ...
    hierarchize_parser.add_argument('inputfolder',
                                    help='path to the directory including single term' +
                                    ' files')
    hierarchize_parser.add_argument('outputfile', help='path to the JSON output file')
    hierarchize_parser.add_argument('hierarchycode',
                                    help='code of the hierarchy to build')
    hierarchize_parser.add_argument('-v', '--verbose', action='count', default=0,
    ...
    if args.command == 'parsexml':
        if (args.nchunks is not None or args.single == True) and args.termsfile == None:
            ap.error('-n/--nchunks and -s/--single can only be used if -t/--termsfile' +
                     ' is declared')
